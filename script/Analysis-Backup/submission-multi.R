...
setwd("/Volumes/jjo11/cascade/CareCascadeV2/2015/November/27th/Multivariate-Final")
...
Interventions <- c("HBCT","Enhanced VCT","HBCT (with POC CD4)","Facilitated Linkage","VCT POC CD4","Pre-ART Outreach","Improved Care","POC CD4","On-ART Outreach","Adherence","Immediate ART","Universal Test & Treat","Optimal Combination","Optimal Combination + Immediate ART")
...
results$Interventions <- factor(results$Interventions, levels=c("HBCT","Enhanced VCT","HBCT (with POC CD4)","Facilitated Linkage","VCT POC CD4","Pre-ART Outreach","Improved Care","POC CD4","On-ART Outreach","Adherence","Immediate ART","Universal Test & Treat","Optimal Combination","Optimal Combination + Immediate ART"))
levels(results$Interventions)
results

results <- mutate(results,label = c("HBCT","VCT","HBCT (with POC CD4)","Linkage","VCT POC CD4","Pre-ART Outreach","Improved Care","POC CD4","On-ART Outreach","Adherence","Immediate ART","Universal Test & Treat","Optimal Combination","Optimal Combination + Immediate ART"))
...
setwd("/Volumes/jjo11/cascade/CareCascadeV2/2015/November/27th/")
system("mkdir ./plots/")
...
# Load up results.
run <- c("run1","run2","run3","run4","run5","run6","run7","run8","run9","run10")
interventions <- c("Baseline", "Hbct_1", "Hbct_2", "Vct_1", "Vct_2", "HbctPocCd4_1", "HbctPocCd4_2", "Linkage_1", "Linkage_2", "VctPocCd4", "PreOutreach_1", "PreOutreach_2", "ImprovedCare_1", "ImprovedCare_2", "PocCd4", "ArtOutreach_1", "ArtOutreach_2", "Adherence_1", "Adherence_2", "ImmediateArt", "UniversalTestAndTreat_1", "UniversalTestAndTreat_2", "Multi_5", "ImmediateArtAndMulti_5")
...
short_interventions <- c("Hbct_2","Vct_2","HbctPocCd4_2","Linkage_2","VctPocCd4","PreOutreach_2","ImprovedCare_2","PocCd4","ArtOutreach_2","Adherence_2","ImmediateArt","UniversalTestAndTreat_2","Multi_5","ImmediateArtAndMulti_5")
...
theScenario <- c("Real","Real","Real","Real","Max","Real","Real","Max","Real","Real","Max","Real","Combo","Combo")
theColor <- c(plotCol[1],plotCol[2],plotCol[3],plotCol[4],plotCol[5],plotCol[6],plotCol[7],plotCol[8],plotCol[9],plotCol[10],plotCol[11],plotCol[12],"grey",1)
...
p <- p + theme(axis.title=element_text(size=8))
p <- p + theme(axis.text=element_text(size=7))
p <- p + theme(legend.position=c(0.44,0.275))
p <- p + theme(legend.title=element_blank())
p <- p + theme(legend.text=element_text(size=7))
p <- p + theme(legend.justification=c(1,0))
p <- p + theme(legend.key = element_blank())
...
graphics.off()
adjustWeight = 0.7
quartz.options(h=8 * adjustWeight,w=7.5 * adjustWeight)
...
# dollar((sum(Baseline$sCOST) * sizeAdjustment)  / 1e+9)
# dollar((sum(ImmediateArt$sCOST) * sizeAdjustment)  / 1e+9)
# dollar((sum(Multi_5$sCOST) * sizeAdjustment)  / 1e+9)
# dollar((sum(UniversalTestAndTreat_2$sCOST) * sizeAdjustment)  / 1e+9)
# dollar((sum(ImmediateArtAndMulti_5$sCOST) * sizeAdjustment)  / 1e+9)
# dollar((sum(UttAndMulti_5$sCOST) * sizeAdjustment)  / 1e+9)
...
tiff(filename = "./plots/Fig4.tiff",
    width = 7.2 * adjustWeight,
    height = 7.9 * adjustWeight,
    units = "in",
    res = 300,
    compression = "lzw",
    bg = "white",
